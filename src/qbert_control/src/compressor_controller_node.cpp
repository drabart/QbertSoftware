...
class CompressorNode : public rclcpp::Node
{
public:
  CompressorNode() : Node("compressor_node")
  {
    pub_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
        "/compressor_controller/commands", 10);

    // Services to trigger actions
    extend_srv_ = this->create_service<std_srvs::srv::Trigger>(
        "extend_compressors",
        std::bind(&CompressorNode::extend_callback, this, std::placeholders::_1, std::placeholders::_2));

    retract_srv_ = this->create_service<std_srvs::srv::Trigger>(
        "retract_compressors",
        std::bind(&CompressorNode::retract_callback, this, std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO(this->get_logger(), "CompressorNode ready");
  }
...
private:
      ...
      std::shared_ptr<std_srvs::srv::Trigger::Response> res)
  {
    ...
    res->success = true;
    res->message = "Compressors extended";
  }
      ...
      std::shared_ptr<std_srvs::srv::Trigger::Response> res)
  {
    ...
    res->success = true;
    res->message = "Compressors retracted";
  }
};
...
int main(int argc, char *argv[])
{
  ...
  rclcpp::spin(std::make_shared<CompressorNode>());
  rclcpp::shutdown();
  return 0;
}
