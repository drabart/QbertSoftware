# FlexBE States and Behaviors for qbert_fsm
...
This raw repository has several folders and files with the generic name `qbert_fsm`.
...
and change the relevant `qbert_fsm` text to `my_new_project_name` as needed.
...
## Example States in `qbert_fsm_flexbe_states`
...
## Example Behaviors in `qbert_fsm_flexbe_behaviors`
...
[`copy_behavior`](qbert_fsm_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run qbert_fsm_flexbe_behavior copy_behavior` to see the usage guide. 
